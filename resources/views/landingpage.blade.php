...
@section('title', 'Beranda Utama - Universitas HKBP Nomensen')
...
<div class="relative bg-gradient-to-br from-slate-900 via-blue-950 to-slate-900 text-white overflow-hidden py-24 sm:py-32">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
        ...
        <div class="mt-10 flex flex-wrap justify-center gap-4">
            <a href="{{ route('profile') }}" class="px-8 py-3.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-sm shadow-lg shadow-blue-500/30 transition-all transform hover:-translate-y-0.5">
                Jelajahi Profil Kampus
            </a>
            <a href="{{ route('lectures') }}" class="px-8 py-3.5 rounded-xl bg-white/10 hover:bg-white/20 text-white border border-white/20 font-bold text-sm transition-all">
                Daftar Dosen
            </a>
        </div>
    </div>
...
<div class="py-20 bg-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        @if($greeting)
            <div class="p-8 sm:p-12 rounded-3xl bg-slate-50 border border-slate-100 shadow-sm max-w-4xl mx-auto">
                ...
                <h3 class="text-2xl font-bold text-slate-900 mb-4">{{ $greeting->title ?? 'Sambutan Rektor' }}</h3>
                <p class="text-slate-600 leading-relaxed text-base">{{ $greeting->content ?? $greeting->description ?? '' }}</p>
                <div class="mt-8 pt-6 border-t border-slate-200/60 flex items-center gap-3">
                    <div class="w-12 h-12 rounded-full bg-blue-100 text-blue-700 font-bold flex items-center justify-center text-lg">R</div>
                    <div>
                        <span class="block font-bold text-slate-900">{{ $greeting->author ?? 'Pimpinan Kampus' }}</span>
        ...
        @else
            <div class="p-12 text-center rounded-3xl bg-slate-50 border border-dashed border-slate-300 max-w-3xl mx-auto">
                <p class="text-slate-500 font-medium">Belum ada data sambutan dari pimpinan.</p>
            </div>
        @endif
    </div>
</div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <div>
                <div class="flex items-center justify-between mb-8">
                    <h3 class="text-2xl font-extrabold text-slate-900">Berita Terkini</h3>
                    <a href="{{ route('news') }}" class="text-sm font-bold text-blue-600 hover:underline">Lihat Semua &rarr;</a>
                </div>
                @if($latestNews && $latestNews->count() > 0)
                    <div class="space-y-6">
                        @foreach($latestNews as $item)
                        <div class="p-6 rounded-2xl border border-slate-200/80 hover:shadow-md transition-shadow">
                            <span class="text-xs font-semibold text-blue-600">{{ $item->created_at ? $item->created_at->format('d M Y') : 'Berita' }}</span>
                            <h4 class="mt-1 text-lg font-bold text-slate-900">{{ $item->title }}</h4>
                            <p class="mt-2 text-sm text-slate-600 line-clamp-2">{{ strip_tags($item->content ?? $item->description ?? '') }}</p>
                        </div>
                        @endforeach
                    </div>
                @else
            ...
            <div>
                <div class="flex items-center justify-between mb-8">
                    <h3 class="text-2xl font-extrabold text-slate-900">Pengumuman Kampus</h3>
                    <a href="{{ route('announcements') }}" class="text-sm font-bold text-blue-600 hover:underline">Lihat Semua &rarr;</a>
                </div>
                @if($latestAnnouncements && $latestAnnouncements->count() > 0)
                    <div class="space-y-6">
                        @foreach($latestAnnouncements as $ann)
                        <div class="p-6 rounded-2xl bg-slate-50 border border-slate-200/80 hover:shadow-md transition-shadow">
                            <span class="text-xs font-semibold text-amber-600">📢 Pengumuman</span>
                            <h4 class="mt-1 text-lg font-bold text-slate-900">{{ $ann->title }}</h4>
                            <p class="mt-2 text-sm text-slate-600 line-clamp-2">{{ strip_tags($ann->content ?? $ann->description ?? '') }}</p>
                        </div>
                        @endforeach
                    </div>
                @else
                    <p class="text-slate-500 text-sm">Belum ada pengumuman terbaru.</p>
                @endif
            </div>
        </div>
    </div>
